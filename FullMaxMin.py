...
class NativeGRUCell(nn.Module):
    def __init__(self, input_dim=1, hidden_dim=2):
        super().__init__()
        # The highly optimized C++ native GRU
        self.gru = nn.GRU(input_size=input_dim, hidden_size=hidden_dim, batch_first=True)
        self.fc_readout = nn.Linear(hidden_dim, 1)

        # Restore the orthogonal initialization from your custom cell
    ...
    def project(self, X: torch.Tensor) -> torch.Tensor:
        # X is [T]. Native GRU expects [Batch, Sequence, Features]
        return X.unsqueeze(0).unsqueeze(-1) # [1, T, 1]

    def readout(self, H: torch.Tensor) -> torch.Tensor:
        # H is [1, T, hidden_dim]. Readout to [1, T, 1] and squeeze down to [T]
        return self.fc_readout(H).squeeze()

class NativeGRURunner(nn.Module):
    def __init__(self, cell):
        super().__init__()
        self.cell = cell
        self.hidden_dim = cell.gru.hidden_size

    def forward(self, X_embeddings: torch.Tensor) -> torch.Tensor:
        # 1. Process the entire sequence in one C++ operation
        out, _ = self.cell.gru(X_embeddings)
        
        # 2. THE CRITICAL FIX: Restore the Filtration Constraint
        # Your custom loop effectively had H[0] = 0, H[1] = f(X[0]), H[2] = f(X[1]).
        # We shift the GRU output to the right by 1 and pad the start with zeros.
        zeros = torch.zeros(1, 1, self.hidden_dim, device=out.device, dtype=out.dtype)
        H_shifted = torch.cat([zeros, out[:, :-1, :]], dim=1)
...
class VectorizedGRUCell(nn.Module):
    ...
    def project(self, X: torch.Tensor) -> torch.Tensor:
        ...
        return self.weight_ih(X.unsqueeze(1)) # X is [T], make it [T, 1]
...
class UnifiedMaxMinSolver:
    def __init__(self, X, cell_instance, runner_script,
        ...
        self.opt_Y = torch.optim.LBFGS([self.Y], lr=lr_Y, max_iter=35   , 
                                       history_size=30, tolerance_change=1e-8, 
...
if __name__ == "__main__":
    ...
    if mode_data == "Kalman":
        ...
        Xsim, Ztrue, epsi = gd.generate_garch_data(T,0.05,0.18,0.75)
        X = torch.tensor(Xsim.reshape(-1), dtype=torch.float32)
        P = np.sqrt(Ztrue) - np.sqrt(Ztrue).mean()
    elif mode_data == "AR":
        Xsim, Ztrue = gd.generate_ar_data(phi_coeffs, sigma2=noise_variance, n_steps=T)
        X = torch.tensor(Xsim.reshape(-1), dtype=torch.float32)
        P = Ztrue
        epsi = Xsim - P
    else:
    ...
    if mode_G == "LINEAR":
        ...
        show_b = True
    elif mode_G== "NATIVE_GRU":
        hidden_dim = 2
        cell = NativeGRUCell(input_dim=1, hidden_dim=hidden_dim)
        runner = NativeGRURunner(cell)
        show_b = False
    ...
    scripted_runner = runner if mode_G == "NATIVE_GRU" else torch.jit.script(runner)
